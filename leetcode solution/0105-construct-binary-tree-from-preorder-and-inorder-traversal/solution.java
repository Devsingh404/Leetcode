...
class Solution {
    ...
    public static TreeNode helper( int[] preorder , int prestart , int preend , int[] inorder , int instart , int inend , Map<Integer,Integer> inmap ){
        ...
        root.left = helper(preorder , prestart+1 , prestart+left , inorder , instart , inroot-1 , inmap);
        root.right = helper(preorder , prestart+left+1 , preend , inorder , inroot+1 , inend , inmap);
